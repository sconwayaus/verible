...
const Symbol *GetInterfaceHeaderNodeFromPortDeclaration(const Symbol &symbol) {
  if (const auto *data_type_node = GetSubtreeAsNode(
          symbol, NodeEnum::kPortDeclaration, 2, NodeEnum::kDataType)) {
    if (const auto *interface_port_header_node =
            GetSubtreeAsNode(*data_type_node, NodeEnum::kDataType, 1)) {
      if (NodeEnum(interface_port_header_node->Tag().tag) ==
          NodeEnum::kInterfacePortHeader) {
...
const SyntaxTreeLeaf *GetInterfaceModPortFromInterfaceHeaderNode(
    const Symbol &symbol) {
