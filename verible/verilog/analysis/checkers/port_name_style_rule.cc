...
static constexpr std::string_view input_style_default_regex =
    "[a-z_0-9]+(_i|_ni|_pi)";
static constexpr std::string_view output_style_default_regex =
    "[a-z_0-9]+(_o|_no|_po)";
static constexpr std::string_view inout_style_default_regex =
    "[a-z_0-9]+(_io|_nio|_pio)";
static constexpr std::string_view interface_style_default_regex = "[a-z_0-9]+";

constexpr std::string_view kInputTitleCase = "Input";
constexpr std::string_view kOutputTitleCase = "Output";
constexpr std::string_view kInoutTitleCase = "Inout";
constexpr std::string_view kInterfaceTitleCase = "Interface";
...
std::string PortNameStyleRule::CreateViolationMessage(
    std::string_view direction, const re2::RE2 &regex) {
...
void PortNameStyleRule::HandleSymbol(const Symbol &symbol,
  ...
  if (PortMatcher().Matches(symbol, &manager)) {
    ...
    if (direction_leaf) {
      ...
      if (modport_leaf) {
        std::string_view modport_suffix = modport_leaf->get().text();
...
absl::Status PortNameStyleRule::Configure(std::string_view configuration) {
